...
### 1. 배경: "new"의 문제점 (The problem with "new")
- 코드에서 `new ConcreteClass()`를 사용하면 구상 클래스(Concrete Class)에 의존하게 된다.
- 이는 **"변하는 부분"**이며, 나중에 코드를 수정해야 할 가능성이 높아진다.
- **OCP(개방-폐쇄 원칙)**를 위반하게 된다. (확장에는 열려 있고 변경에는 닫혀 있어야 한다.)
- **해결책:** 객체 생성 코드를 캡슐화(Encapsulate)해야 한다.
...
> *엄밀히 말하면 디자인 패턴이 아니라 프로그래밍 관용구(Idiom)에 가깝다.*
...
### 3. 핵심 디자인 원칙: 의존성 역전 원칙 (DIP)
> **"추상화된 것에 의존하도록 만들어라. 구상 클래스에 의존하지 않도록 만들어라."**
> *(Depend upon abstractions. Do not depend upon concrete classes.)*

- **의미:** 고수준 구성요소(`PizzaStore`)가 저수준 구성요소(`NYCheesePizza`)에 직접 의존하면 안 된다. 둘 다 **추상화(`Pizza`)**에 의존해야 한다.
- **효과:** 의존 관계의 화살표가 위에서 아래로 내려오다가, 추상화를 만나면서 **역전(Inversion)**된다.
- **가이드라인:**
    - 변수에 구상 클래스의 레퍼런스를 저장하지 말 것.
    - 구상 클래스에서 유도된 클래스를 만들지 말 것.
    - 베이스 클래스에 이미 구현된 메서드를 오버라이드하지 말 것.
...
| :--- | :--- | :--- |
| **생성 대상** | **제품 하나** (One Product) | **제품군** (Family of Products) |
| **구현 방법** | **상속 (Inheritance)**<br>(서브클래스가 결정) | **구성 (Composition)**<br>(팩토리 객체를 주입받아 위임) |
| **유연성** | 클라이언트와 구체적 클래스 분리 | 제품군을 통째로 교체하기 쉬움 |
| **코드 예시** | `abstract createPizza()` | `interface createDough(), createSauce()...` |